...
export default function ProfileAbout({ driver }: ProfileAboutProps) {
    ...
    return (
        <div className="bg-white border border-gray-100 shadow-soft rounded-[30px] sm:rounded-[40px] p-5 sm:p-8 lg:p-12">
            <section className="mb-8 sm:mb-10">
                <div className="flex items-center gap-3 mb-4 sm:mb-6">
                    <div className="bg-blue-50 p-2 rounded-xl">
                        <Award className="h-5 w-5 text-aviva-primary" />
                    </div>
                    <h3 className="text-lg sm:text-xl font-bold tracking-tight text-aviva-navy font-display">Experiencia Profesional</h3>
                </div>
                <div className="p-5 sm:p-6 bg-gray-50 border border-gray-100 rounded-3xl">
                    <p className="text-aviva-subtext text-base sm:text-lg leading-relaxed font-medium">
                        {driver.bio}
                    </p>
                </div>
            </section>

            <section className="mb-8 sm:mb-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
                <div className="flex items-center gap-3 mb-4 sm:mb-6">
                    <div className="bg-purple-50 p-2 rounded-xl">
                        <User className="h-5 w-5 text-purple-600" />
                    </div>
                    <h3 className="text-lg sm:text-xl font-bold tracking-tight text-aviva-navy font-display">Sobre mí</h3>
                </div>
                <div className="p-5 sm:p-6 bg-gray-50 border border-gray-100 rounded-3xl">
                    <p className={`text-base sm:text-lg leading-relaxed font-medium ${driver.personal_bio ? 'text-aviva-subtext' : 'text-gray-400 italic'}`}>
                        {driver.personal_bio || "Este conductor aún no ha redactado su reseña personal."}
                    </p>
                </div>
            </section>
            ...
            <section className="mb-0">
                <div className="flex items-center justify-between mb-4 sm:mb-6">
                    <div className="flex items-center gap-3">
                        ...
                        <h3 className="text-lg sm:text-xl font-bold tracking-tight text-aviva-navy font-display">Vehículo Registrado</h3>
                    </div>
                    {photos.length > 0 && (
                    ...
                    <div className="relative aspect-[16/9] w-full bg-gray-100 overflow-hidden">
                            ...
                            <>
                                <AnimatePresence mode="wait">
                                    <motion.img
                                        ...
                                        alt={`${driver.vehicle} - ${['Frente', 'Lateral', 'Trasera', 'Interior 1', 'Interior 2', 'Interior 3'][selectedPhoto] || `Foto ${selectedPhoto + 1}`}`}
                                    />
                                </AnimatePresence>
                        ...
                        <div className="absolute bottom-6 left-8 text-white z-10">
                            <div className="flex items-center gap-2 mb-1">
                                <span className="bg-aviva-primary/80 backdrop-blur-sm text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider shadow-lg">
                                    {['Frente', 'Lateral', 'Trasera', 'Interior 1', 'Interior 2', 'Interior 3'][selectedPhoto] || `Foto ${selectedPhoto + 1}`}
                                </span>
                            </div>
                        ...
                        <div className="p-4 bg-gray-50/50 backdrop-blur-sm border-t border-gray-100">
                            <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide px-2">
                                {photos.map((photo, idx) => (
                                    ...
                                    >
                                        <img src={photo} className="w-full h-full object-cover" alt={['Frente', 'Lateral', 'Trasera', 'Interior 1', 'Interior 2', 'Interior 3'][idx] || `Foto ${idx + 1}`} />
                                    </button>
                                ))}
                            </div>
                        </div>
